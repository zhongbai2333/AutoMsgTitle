...
'''.format("!!amt", 5, self.__mcdr_server.get_self_metadata().version).splitlines(True)
    ...
    def regions_list(self, source: CommandSource, context: CommandContext):
        ...
        def get_regions_rtext(regions: dict, page: int):
            ...
            regions_rtext.append(f"--------- 区域列表 第 §6{page}/{len(regions) // 5 + 1} §f页 ---------\n")
            num = 0
            for i in regions.keys():
                ...
                regions_rtext.append(f" | 消息栏：\n")
                for j in regions[i]['msg']['msg']:
                    regions_rtext.append(f" |  | §6{j}\n")
                    reload_command += f"{j};;"
                regions_rtext.append("---")
                regions_rtext.append(RText("§7[↺]§r").c(RAction.suggest_command, reload_command[:-2]).h('重新键入 §7!!amt add §r命令'))
                regions_rtext.append("---")
                regions_rtext.append(RText("§6[✏]§r").c(RAction.run_command, f"!!amt msg {i}").h("详细编辑消息栏"))
                regions_rtext.append("---")
                regions_rtext.append(RText("§4[✕]§r").c(RAction.run_command, f"!!amt del {i}").h("删除这个区域"))
                regions_rtext.append("---\n")
            regions_rtext.append("--------- ")
            regions_rtext.append(RText("<<").c(RAction.run_command, f"!!amt list {page - 1}").h("前一页"))
            regions_rtext.append(f" 第 §6{page}/{len(regions) // 5 + 1} §f页 ")
            regions_rtext.append(RText(">>").c(RAction.run_command, f"!!amt list {page + 1}").h("后一页"))
            regions_rtext.append(" ---------")
    ...
    def add_region_2d(self, source: CommandSource, context: CommandContext):
        ...
        msg = context['msg'].replace(f"[{title}]", '').strip().replace(f"({subtitle})", '').strip().replace(f"#{actionbar}#", '').strip().split(";;")
    ...
    def del_region(self, source: CommandSource, context: CommandContext):
        ...
        if self.ready_del != context['region_name']:
            ...
            del_msg_rtext.append(f"重复输入 §7!!amt del {context['region_name']} §r以确认！")
            del_msg_rtext.append(RText("§a[✓]").c(RAction.run_command, f"!!amt del {context['region_name']}").h("§a确认删除"))
